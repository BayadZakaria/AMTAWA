...
export default function Scanner({ medicalProfile, user, onUpdateUser, language = 'en' }: ScannerProps) {
  ...
  const getAdditivesImpact = (count: number) => {
    if (count === 0) return { label: language === 'fr' ? 'Aucun additif' : language === 'ar' ? 'بدون إضافات' : language === 'zgh' ? 'ⵡⴰⵍⵓ ⵉⵙⵎⴰⵜⵜⴰⵢⵏ' : 'No additives', color: 'text-[#038141]', bg: 'bg-green-50', border: 'border-green-200' };
    if (count <= 2) return { label: language === 'fr' ? 'Impact faible' : language === 'ar' ? 'تأثير ضعيف' : language === 'zgh' ? 'ⵉⵎⵉⴽⴽ ⵏ ⵓⵙⵎⴰⵜⵜⴰⵢ' : 'Low additives impact', color: 'text-yellow-600', bg: 'bg-yellow-50', border: 'border-yellow-200' };
    if (count <= 4) return { label: language === 'fr' ? 'Impact modéré' : language === 'ar' ? 'تأثير متوسط' : language === 'zgh' ? 'ⴰⵙⵎⴰⵜⵜⴰⵢ ⴰⵏⴰⵎⵎⴰⵙ' : 'Moderate impact', color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-200' };
    return { label: language === 'fr' ? 'Impact élevé' : language === 'ar' ? 'تأثير عالي' : language === 'zgh' ? 'ⴰⵙⵎⴰⵜⵜⴰⵢ ⴰⵅⴰⵜⴰⵔ' : 'High additives impact', color: 'text-[#E63E11]', bg: 'bg-red-50', border: 'border-red-200' };
  };
