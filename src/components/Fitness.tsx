...
export default function Fitness({ user, medicalProfile, onUpdateUser, language = 'en' }: FitnessProps) {
  ...
  const handleGenerateFitness = async (isTranslationRequest = false) => {
    if (!isTranslationRequest && (user.tokens || 0) < 1) {
    ...
    try {
      ...
      if (user.isMock || !supabase) {
        ...
        hist.fitness.push({ ...data, date: new Date().toISOString() });
      ...
      if (!isTranslationRequest) {
        onUpdateUser({
           ...user,
           tokens: Math.max(0, (user.tokens || 0) - 1),
           fitnessPlan: data
        });
      } else {
        onUpdateUser({
      ...
      if (!isTranslationRequest) setError(err.message || 'An error occurred while generating your fitness plan.');
    } finally {
      if (!isTranslationRequest) setLoading(false);
    }
  };

  React.useEffect(() => {
    if (user.fitnessPlan || generatedPlan) {
      handleGenerateFitness(true);
    }
  }, [language]);
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
        ...
        {!generatedPlan && !loading && (
           <div className="py-8 flex flex-col items-center justify-center text-center">
              ...
              <button 
                onClick={() => handleGenerateFitness(false)}
