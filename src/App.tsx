...
export default function App() {
  ...
  useEffect(() => {
    if (!user?.id || !supabase || user.isMock) return;
    ...
    const loadData = async () => {
      ...
      if (!userData) {
         // Create default user record using upsert to avoid conflicts
      ...
      if (userData) {
         ...
         if (lastLogin !== today) {
           userData.tokens = Math.max(userData.tokens || 0, 50);
           await supabase.from('users').update({ tokens: userData.tokens }).eq('id', user.id);
      ...
      if (userData) {
        ...
        if (medData) {
           ...
           if (medError) {
             console.error("Error creating medical profile:", medError.message || medError);
           }
        }
      }
      ...
      try {
        ...
        if (notifRes.ok) {
          const _text = await notifRes.text();
          let notifData;
          try { notifData = JSON.parse(_text); } catch(e) { throw new Error(_text.slice(0, 50)); }
  ...
  const handleUpdateUser = async (updates: Partial<UserProfile>) => {
    ...
    if (supabase && user?.id && !user.isMock) {
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-purple-50 to-slate-100 flex flex-col font-sans text-slate-800 ${isRTL ? 'text-right' : 'text-left'}`} dir={isRTL ? 'rtl' : 'ltr'}>
              ...
              <>
                ...
                <div className={`absolute top-full lg:right-0 mt-2 bg-white text-slate-800 rounded-xl shadow-lg border border-slate-100 overflow-hidden w-[280px] sm:w-80 max-h-96 z-50 flex flex-col right-0`}>
                  ...
                  <div className="overflow-y-auto flex-1">
                    {(notifications?.length || 0) > 0 ? notifications.map(notif => (
      ...
      {/* Main Content */}
      <main className="flex-1 w-full max-w-5xl mx-auto p-4 md:p-6 mb-32 md:mb-6 pt-4 md:pt-6">
        {renderTab()}
      </main>

      {/* Bottom Navigation for Mobile */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-xl border-t border-purple-100 shadow-[0_-10px_40px_rgba(168,85,247,0.15)] pb-[env(safe-area-inset-bottom)]">
        <nav className="flex justify-between items-start pt-3 pb-2 px-1 w-full min-h-[5.5rem]">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                id={`nav-btn-${item.id}`}
                className={`flex flex-col items-center justify-start px-0.5 rounded-xl transition-all flex-1 relative h-full ${
                  isActive ? 'text-purple-700' : 'text-slate-400 hover:text-purple-500'
                }`}
              >
                {isActive && (
                   <span className="absolute -top-3 w-8 h-1.5 bg-purple-600 rounded-b-full"></span>
                )}
                <div className={`p-2 rounded-2xl mb-1.5 transition-all duration-300 ${isActive ? 'bg-purple-100 scale-110 shadow-sm' : 'bg-transparent'}`}>
                  <Icon className={`w-6 h-6 ${isActive ? 'animate-bounce-short text-purple-700' : ''}`} />
                </div>
                <span className={`text-[10px] leading-[1.1] font-bold tracking-wide text-center whitespace-normal w-full break-words ${isActive ? 'text-purple-800' : 'text-slate-500'}`}>
                  {item.label}
                </span>
              </button>
            );
